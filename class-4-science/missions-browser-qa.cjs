...
const topics=await p.evaluate(()=>SAFARI.topics.filter(t=>!t.campaign).map(t=>({id:t.id,name:t.name})));let scenes=0;
...
await p.goto(base);assert.equal(await p.locator('#chapter-grid .class4-card').count(),10);await p.locator('#chapter-grid .class4-card').nth(9).click();await p.locator('.world-title h2').filter({hasText:'Our Sky'}).waitFor();
const result={games:topics.length,viewports:5,scenes,articles:topics.length,layout,svgBounds,errors,assets,drawing:'passed',restart:'passed',legacyBookmarks:'passed',chapters:10};fs.writeFileSync('/tmp/science-v2-browser-results.json',JSON.stringify(result,null,2));console.log(JSON.stringify(result));await b.close();if(layout.length||errors.length||assets.length||svgBounds.length)process.exitCode=1;
